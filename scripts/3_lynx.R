# Session 3: Eurasian lynx reintroduction
...
#### + 0.05
# Transition matrix
(trans_mat_new <- matrix(c(0,1,0,0,0,0, 0.53, 0,0, 0, 0, 0.63, 5,0, 0, 0.8*1.05), nrow = 4, byrow = F))  
# stage 0: dispersing newborns; stage 1 : first-year juveniles; stage 2: non-breeding sub-adults; stage 3: breeding adults

# Define the stage structure
stg_new <- StageStructure(Stages = 4, # four stages including dispersing juveniles
                      TransMatrix = trans_mat_new,  # transition matrix
                      MaxAge = 17,  # maximum age
                      FecDensDep = T   # density dependence in fecundity
)
...
# Define the demography module
demo_new <- Demography(StageStruct = stg_new, 
                   ReproductionType = 1, # simple sexual model
                   PropMales = 0.5) 


s_29_new <- RSsim(batchnum = 20, land = land, demog = demo_new, dispersal = disp, simul = sim, init = init_29, seed = 324135)
RunRS(s_29_new, dirpath)


#### - 0.05
# Transition matrix
(trans_mat_new1 <- matrix(c(0,1,0,0,0,0, 0.53, 0,0, 0, 0, 0.63, 5,0, 0, 0.8*0.95), nrow = 4, byrow = F))  
# stage 0: dispersing newborns; stage 1 : first-year juveniles; stage 2: non-breeding sub-adults; stage 3: breeding adults

# Define the stage structure
stg_new1 <- StageStructure(Stages = 4, # four stages including dispersing juveniles
                          TransMatrix = trans_mat_new1,  # transition matrix
                          MaxAge = 17,  # maximum age
                          FecDensDep = T   # density dependence in fecundity
)
...
# Define the demography module
demo_new1 <- Demography(StageStruct = stg_new1, 
                       ReproductionType = 1, # simple sexual model
                       PropMales = 0.5) 


s_29_new1 <- RSsim(batchnum = 21, land = land, demog = demo_new1, dispersal = disp, simul = sim, init = init_29, seed = 324135)
RunRS(s_29_new1, dirpath)
...
abund_sens %>%
  ...
  geom_ribbon(aes(x=Year, ymin=Abundance-sd, ymax=Abundance+sd, color=Scenario), linetype=2, alpha=0.1)
  
  
pop_1 <- readPop(s_29, dirpath)
extProb_pop1 <- Calc_ExtProb(pop_1, s_29)
Calc_ExtTime(pop_1)
  
pop_2 <- readPop(s_29_new, dirpath)
extProb_pop2 <- Calc_ExtProb(pop_2, s_29_new)
Calc_ExtTime(pop_2)

pop_3 <- readPop(s_29_new1, dirpath)
extProb_pop3 <- Calc_ExtProb(pop_3, s_29_new1)
Calc_ExtTime(pop_3)


extProb_sens <- bind_rows(extProb_pop1 %>% add_column(Scenario = "1 - Default"),
                          extProb_pop2 %>% add_column(Scenario = "2 - AdultSurv +5%"),      
                          extProb_pop3 %>% add_column(Scenario = "3 - AdultSurv -5%"))
  

extProb_sens %>%
  ggplot() +
  geom_line(aes(Year, extProb, color=Scenario), size=2)
  geom_ribbon(aes(x=Year, ymin=Abundance-sd, ymax=Abundance+sd, color=Scenario), linetype=2, alpha=0.1)
